...
ACLNN_NOTES = {
    ...
    ('quant', 'per_channel_cast_and_transpose'): 'No torch_npu fused cast+transpose kernel',
    ('quant', 'per_block_cast_lossless'): 'row_block_size=32 unsupported on Ascend 950 (only 1/128/256/512); no FP8 anti_quant',
    ('quant', 'cast_back_e5m6'): 'No torch_npu API for e5m6 custom format',
    ('quant', 'per_token_cast_to_e5m6'): 'No torch_npu API for e5m6 custom format',
    ('quant', 'swiglu_backward_and_per_token_cast'): 'No SwiGLU backward API in torch_npu',
}

ACLNN_SUPPORTED_NOTES = {
    ('quant', 'per_token_cast'): 'npu_dynamic_block_quant (FP8 e4m3, row=1) on Ascend 950; npu_dynamic_quant (int8) fallback',
    ('quant', 'per_channel_cast'): 'npu_dynamic_quant(quant_mode="perchannel") int8 output',
    ('quant', 'per_channel_cast_fused'): 'npu_dynamic_quant + gather preprocessing',
    ('quant', 'per_block_cast'): 'npu_dynamic_block_quant(dst_type=292 FP8 e4m3fn) on Ascend 950; row ∈ {1,128,256,512}, col ∈ {64,128,192,256}',
    ('quant', 'cast_back'): 'npu_anti_quant (int8 -> bf16/fp16) or manual torch dequant for FP8 input',
    ('quant', 'swiglu_forward_and_per_channel_cast_and_transpose'): 'npu_swiglu_quant(quant_mode=0, dst_type=int8)',
    ('quant', 'swiglu_forward_and_per_token_cast'): 'npu_swiglu_quant(quant_mode=1, dst_type=int8, per-token dynamic)',
